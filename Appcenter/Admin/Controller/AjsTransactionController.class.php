...
class AjsTransactionController extends AuthController {
	
	private $AjsTransactionModel;
	
	public function __construct($request='') {
		parent::__construct(false, $request);
		
		$this->AjsTransactionModel = new TransactionsModel();
	}
	...
	public function index($tpl='') {
		...
		$type = 'AJS';
		$platform = empty($this->get['platform']) ? 'AGX' : $this->get['platform'];
		...
		switch ($platform) {
			case 'AJS':
				$where['ua.wallet_address'] = ['exp', '=t.address'];
				break;
			case 'AGX':
				$where['ua.wallet_address_2'] = ['exp', '=t.address'];
				break;
		}
		
		$data = $this->AjsTransactionModel->getAoList('t.*', $page, 20, $where);
		...
		$score_receive = $this->AjsTransactionModel->getInfo('sum(amount) amount', $map_score);
		$score_reward = $this->AjsTransactionModel->getInfo('sum(reward_amount) reward_amount', $map_score);
	...
	public function tradeAction() {
		...
		foreach ($checkList as $k=>$v) {
			...
			//奖励金额
			$reward_bai = M('Settings')->where("settings_code='ajs_received_bai'")->getField('settings_value');
