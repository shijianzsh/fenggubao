    ...
    REPEAT
      ...
      THEN
        BEGIN
          out_repeat:
          BEGIN
            SET @portion = floor((c_consume_amount - c_consume_amount_old) / @performancePortionBase);
  ...
  BEGIN
    ...
    BEGIN
      START TRANSACTION
        ...
        -- 验证是否已经开始挖矿
        SET @startHour = GetSetting(CONCAT('mine_start_hour'));
        IF from_unixtime(unix_timestamp(), '%H') - @startHour < 0
        THEN
          LEAVE out_label;
        END IF;

        -- 验证是否已经结束挖矿
        SET @endHour = GetSetting(CONCAT('mine_end_hour'));
        IF from_unixtime(unix_timestamp(), '%H') - @endHour >= 0
        THEN
          LEAVE out_label;
        END IF;
