...
class Verify {
    protected $config =	array(
        ...
        'angle'     =>  true,           //验证码字体是否旋转
        'codeType'  =>  false,          //设置数字类型(可选:string,number)
        'codeUL'    =>  false,          //设置大小写(可选:lower,upper)
        'codeNX'    =>  false,           //是否启用指定字符左边距,赋值则按值设置间距
        );
    ...
    public function entry($id = '') {
        ...
        //设置字体旋转
        $angle = $this->angle ? mt_rand(-40,40) : 0;
        ...
        if ($this->codeUL) {
        	switch ($this->codeUL) {
        		case 'lower':
        			$this->codeSet = strtolower($this->codeSet);
        			break;
        		case 'upper':
        			$this->codeSet = strtoupper($this->codeSet);
        			break;
        	}
        	//去除重复的字符
        	$this->codeSet = implode('', array_unique(str_split($this->codeSet)));
        }
        ...
        if($this->useZh){ // 中文验证码
            for ($i = 0; $i<$this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, $angle, $this->fontSize*($i+1)*1.5, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->length; $i++) {
                $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet)-1)];
               	$codeNX  += $this->codeNX ? $this->codeNX : mt_rand($this->fontSize*1.2, $this->fontSize*1.6);
                imagettftext($this->_image, $this->fontSize, $angle, $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
