...
class Model {
     ...
     * @param boolean $auto_key 是否有自增主键ID,默认无
     * @return mixed
     */
    public function add($data='',$options=array(),$replace=false, $auto_key=false) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if(!empty($this->data)) {
                $data           =   $this->data;
     ...
     * @param boolean $auto_key 是否有自增主键ID,默认有
     * @return mixed
     */
     public function create($data='',$type='', $auto_key=true) {
        // 如果没有传值默认取POST数据
        if(empty($data)) {
            $data   =   I('post.');
    ...
    /**
     * @yinhexi: 获取新插入值的ID (用于非自增主键ID)
     */
    public function getInsertId() {
    	$list = $this->lock(true)->field('id')->order('id desc')->limit(1)->find();
    	$insert_id = count($list)>0 ? $list['id'] : 1;
    	return $insert_id;
    }
