...
class Db {
    ...
    static private function parseConfig($config){
        ...
        if (C('DB_PWD_SAFE')) {
        	...
        	//判断是否为分布式数据库+判断是否为多个密码(依据半角逗号区分)
        	if (C('DB_DEPLOY_TYPE') == '1' && preg_match('/(.*),(.*)/', $config['password'])) {
        		$config_password = explode(',', $config['password']);
        		$config_password_decrypt = array();
        		foreach ($config_password as $k=>$pwd) {
        			$config_password_decrypt[$k] = $Crypt->decrypt($pwd, C('ADMINISTRATOR_VAR'), true);
        		}
        		$config['password'] = implode(',', $config_password_decrypt);
        	} else {
        		$config['password'] = $Crypt->decrypt($config['password'], C('ADMINISTRATOR_VAR'), true);
        	}
        }
        return $config;
    }
