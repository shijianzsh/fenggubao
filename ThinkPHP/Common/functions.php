...
function load_config($file,$parse=CONF_PARSE){
    ...
    switch($ext){
        case 'php':
        	//特殊处理:parameter.php中配置参数由于在负载均衡中会出现不同步情况,故在此直接读取parameter表中extend_data字段中值
        	if (preg_match('/\/Conf\/parameter.php$/', $file)) {
        		$parameter_data = M('Parameter', 'g_')->field('extend_data')->where('id=1')->find();
        		if (!empty($parameter_data['extend_data'])) {
        			return json_decode($parameter_data['extend_data'], true);
        		}
        	}
            return include $file;
        case 'ini':
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(isset($host)) {
        ...
        //智能判断是否已开启附件分离模式,若已开启则自动使用第一个附件域名作为domain(此功能排除对APP模块的改动)
        if (C('ATTACH_SEPARATION_ON') && MODULE_NAME!='APP') {
        	$domain = preg_replace('/^(http:\/\/|https:\/\/)?/', '', C('DEVICE_CONFIG')[C('DEVICE_CONFIG_DEFAULT')]['attach_domain'][0]);
        }
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
        	...
        	//@yinhexi:优化多兼容
        	/*
            if (preg_match('/^(127.0.0.1|localhost){1,}/', $domain)) {
            	$domain = $domain;
            } else {
            	$domain = 'www'.strstr($domain,'.');
            }
            */
            // '子域名'=>array('模块[/控制器]');
            foreach (C('APP_SUB_DOMAIN_RULES') as $key => $rule) {
                ...
                if(false === strpos($key,'*') && 0=== strpos($url,$rule)) {
                	...
                	//兼容多级(二级以上)域名
                	preg_match('/^(http:\/\/)?([0-9a-zA-Z.]+)(\/(.*))?$/', $domain, $matches);
                	$host = $matches[2]; //example: cs.api.zcsh123.com
                	$host = explode('.', $host);
